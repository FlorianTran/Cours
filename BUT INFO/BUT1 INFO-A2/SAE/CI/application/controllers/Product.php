...
class Product extends CI_Controller {
	...
	public function display(int $id){
		...
		$this->load->view('displayProduct',  array('product' => $product, 'allcartproducts' => $allProductsOfCart));
	}
	...
	public function decreaseProductQuantity(int $id, int $qtyToDecrement){
		$product=$this->ProductModel->findById($id);
		$new_qty=$product->getQuantity() - $qtyToDecrement;
		if ($product->getQuantity()>=$qtyToDecrement){
			$this->ProductModel->update($id, $product->getName(), $product->getPrice(), $new_qty, $product->getPicturesString());
		}
	...
	public function syncCartAndDatabase(){
		$cart=$this->ProductModel->getAllCartArray();
		foreach ($cart as $product){
			$this->decreaseProductQuantity($product[0],$product[1]);
		}
		delete_cookie("cart");
		$cart=$this->ProductModel->getAllCartArray();
...
// --------------------------------CART FUNCTIONS-------------------------------//
	...
	public function updateCartQty(int $id, int $new_qty){
		$cart=$this->ProductModel->getAllCartArray();
		$product=$this->ProductModel->findById($id);
		if ($product->getQuantity()<$new_qty or 0>=$new_qty){
			$new_qty=$product->getQuantity();
		}
		$index=0;
		for ($i=0; $i<sizeOf($cart); $i++){
			if ($cart[$i][0]==$id){
